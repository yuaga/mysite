...
class Captcha(object):
    ...
    SOURCE = list(string.ascii_letters)  # 返回包含小写字母大写字母的列表
    ...
    @classmethod
    def gene_text(cls):
        # sample取样，从SOURCE中选4次，number是生成验证码的位数
        return ''.join(random.sample(cls.SOURCE, cls.number))

    # 用来绘制干扰线
    @classmethod
    def __gene_line(cls, draw, width, height):
        # 相当于一个x,y轴上找两个点，确定开始点，结束点
    ...
    @classmethod
    def gene_code(cls):
        width, height = cls.size  # 从size中拿出宽和高
        # 创建画板，A代表透明度，然后宽和高， 背景颜色
        image = Image.new('RGBA', (width, height), cls.bgcolor)
        # 验证码的字体路径，字体大小
        font = ImageFont.truetype(cls.font_path, cls.fontsize)
        draw = ImageDraw.Draw(image)  # 创建画笔，用来在image上产生验证码
        text = cls.gene_text()  # 生成字符串
        font_width, font_height = font.getsize(text)  # 得到text文本的宽度，高度
        # 填充字符串，文本的位置，文本内容，文本字体，字体颜色
